...
def get_all_run_params():
    ...
    # Load Pareto-optimal runs from JSON
    pareto_runs = pd.read_json("pareto_runs.json", orient="records")

    # Expand each parameter tuple `iters` times
    expanded_params = [
        row
        for row in pareto_runs[param_cols].itertuples(index=False, name=None)
        for _ in range(iters)
    ]

    # Assign unique run_ids
    return [(i, *params) for i, params in enumerate(expanded_params)]
...
def single_run(args):
    run_id, std_scale, height_frac, perturb, opt = args
