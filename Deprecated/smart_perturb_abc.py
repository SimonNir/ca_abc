import numpy as np
from deprecated.traditional_abc import GaussianBias, TraditionalABC

class SmartPerturbABC(TraditionalABC):

    def __init__(self, *args, bias_curve_scale_factor, **kwargs): 
        super().__init__(*args, **kwargs)
        self.bias_curve_scale_factor = bias_curve_scale_factor
    ...
    def compute_hessian_finite_difference(self, position, eps=1e-3):
        ...
        return -np.array(hessian, dtype=float)  # negative because we're working with -∇V
    ...
    def conjugate_directions(self, hessian):
        ...
        return zip(eigvals, eigvecs)
    
    def compute_exact_softest_hessian_mode(self, hessian):
        """
        ...
        """
        softest = sorted(self.conjugate_directions(hessian), key=lambda x: x[1])[0]
    ...
    def make_positive_definite(H, eps=1e-6):
        """ 
        Modifies the passed H to ensure positive definite. 
        Used to estimate bias covariance from hessian.
        """
        H = np.atleast_2d(H)
        eigvals, eigvecs = np.linalg.eigh(H)
        eigvals_mod = np.clip(eigvals, eps, None)
        H_mod = eigvecs @ np.diag(eigvals_mod) @ eigvecs.T
        return H_mod
    ...
    def run_simulation(
        ...
        for iteration in range(max_iterations):
            ...
            # TODO: implement multiple hessian types
            ...
            if converged:
                if full_hessians:
                    ...
                    covar = self.bias_curve_scale_factor * np.linalg.inv(self.make_positive_definite(hessian))
                else:
                    mode, curve = self.estimate_softest_hessian_mode(self.position)
                    covar = np.abs(self.bias_curve_scale_factor / curve)

                self.perturb_along_softest_mode(perturb_scale, mode)

            if covar is None: 
                covar = np.square(self.default_bias_sigma)
            self.deposit_bias(pos, covar)
...
def run_1d_simulation():
    ...
    abc = SmartPerturbABC(
        ...
        optimizer="CG",
        bias_curve_scale_factor=1
    )
    
    abc.run_simulation(max_iterations=50, perturb_scale=.05, perturb_type="smart", verbose=True, full_hessians=True)
    ...
    plot_results(abc, save_plots=True, filename="1d_smart_abc.png")
...
def run_2d_simulation():
    ...
    abc = SmartPerturbABC(
        ...
        default_bias_sigma=0.5,
        starting_position=[0, 0],
        optimizer='CG',
        bias_curve_scale_factor=1
    )
    
    abc.run_simulation(max_iterations=150, perturb_scale=0.05, perturb_type="random", full_hessians=True, verbose=True)
    ...
    analyze_basin_visits(abc)
    plot_results(abc, save_plots=True, filename="2d_smart_abc.png")
