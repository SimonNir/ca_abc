...
def get_all_run_params():
    ...
    max_height_scales = np.round(np.linspace(1, 7, num=10), 6).tolist()
    max_cov_scales = np.round(np.linspace(1, 7, num=10), 6).tolist()
    iters = 10
    
    base_params = list(product(std_dev_scales, bias_height_fractions, 
                            perturbations, optimizers, max_height_scales, max_cov_scales))
...
def single_run(args):
    ...
    try:
        ...
        run_data = {
            ...
            'max_height_scale':adaptive_height_scale,
            'max_cov_scale':adaptive_cov_scale,
        }
