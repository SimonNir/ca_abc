...
class FIREOptimizer(Optimizer):
    def __init__(self, abc_sim,
                 ...
                 max_steps=1000,
                 f_tol=1e-4,
                 max_step_size=0.05,     # default leash like your working version
                 velocity_damping=0.9):  # gentle damping like before
        """
        FIRE Optimizer in the "worked" update order:
        - If P <= 0: zero v and shrink dt BEFORE stepping (prevents bad steps)
        - Optional velocity damping every step
        - Optional per-step displacement cap via max_step_size

        Units: assumes your x and grad/forces are consistent (e.g., Å and eV/Å).
        """
    ...
    def _run_optimization(self, x0, max_steps=None, convergence_threshold=None, min_steps=None, verbose=False):
        ...
        for step in range(max_steps):
            ...
            if np.any(np.isnan(force)):
                if verbose:
                    print("NaN detected in force! Resetting velocity and skipping step.")
                self.v[:] = 0.0
                force[:] = 0.0

            fmax = float(np.max(np.abs(force)))
            if verbose:
                print(f"Step {step}: E = {energy:.6f} eV, |F|_max = {fmax:.6e} eV/Å, dt = {dt:.4f}, α = {alpha:.4f}")
            ...
            # 2) power
            P = float(np.dot(force, self.v))

            # 3) velocity projection
            v_norm = float(np.linalg.norm(self.v))
            f_norm = float(np.linalg.norm(force))
            if v_norm > 1e-20 and f_norm > 1e-20:
                self.v = (1.0 - alpha) * self.v + alpha * force * (v_norm / f_norm)

            # 4) adaptive control (BEFORE stepping) — this is the key to your old stability
            if P > 0.0:
                n_pos += 1
                if n_pos > N_min:
                ...
                # backtrack behavior: kill momentum and shrink dt before moving
                self.v[:] = 0.0
                dt *= f_dec
                alpha = self.alpha
                n_pos = 0

            # 5) optional velocity damping
            if velocity_damping is not None and velocity_damping < 1.0:
                self.v *= velocity_damping

            # 6) propose step (after corrections)
            step_vec = dt * self.v

            # 7) clip displacement if requested
            if max_step_size is not None:
                step_norm = float(np.linalg.norm(step_vec))
                if step_norm > max_step_size and step_norm > 0.0:
                    step_vec *= (max_step_size / step_norm)
        ...
        # Optional Hessian (if you maintain these arrays elsewhere)
        hess_inv = None
        if hasattr(self, "trajectory") and len(self.trajectory) >= 2:
            try:
...
class ASEOptimizer(Optimizer):
    ...
    def _run_optimization(self, x0, max_steps=None, convergence_threshold=None, min_steps=None, verbose=False):
        ...
        if isinstance(self.abc_sim.potential, ASEPotential):
            ...
            # For non-ASE potentials: map d-dimensional vector to fake ASE atoms
            # Example: put all degrees of freedom on hydrogens in 3D
            n_atoms = int(np.ceil(dim / 3))
            symbols = 'H' * n_atoms
            pos = np.zeros((n_atoms, 3))
            flat_x = np.zeros(n_atoms * 3)
            flat_x[:dim] = x0  # fill only first `dim` coords
            pos = flat_x.reshape(-1, 3)

            atoms = Atoms(symbols, positions=pos)

        atoms.calc = _ASECalculatorWrapper(self, dim)
        ...
        def callback():
            current_pos = self._get_current_x()
    ...
    def _get_current_x(self):
        """Extracts only the relevant DOFs from ASE atoms back to the PES dimension."""
        pos = self._current_atoms.get_positions().flatten()
        return pos[:len(self._last_accepted_pos)]  # truncate padding
...
class _ASECalculatorWrapper(Calculator):
    ...
    def __init__(self, optimizer, dim):
        super().__init__()
    ...
    def calculate(self, atoms=None, properties=['energy'], system_changes=all_changes):
        ...
        current_pos = atoms.get_positions().flatten()[:self.dim]
        energy, grad = self.optimizer._compute(current_pos)

        # Pad gradient into (natoms, 3) shape for ASE forces
        n_atoms = len(atoms)
        forces = np.zeros((n_atoms * 3,))
        forces[:self.dim] = -grad
