...
def get_all_run_params():
    std_dev_scales = [0.170998]
    bias_height_fractions = [0.119897]
    ...
    max_height_scales = np.round(np.linspace(1,3,5)[1:], 6).tolist()
    min_height_scales =  np.round(np.linspace(1,3,5)[1:], 6).tolist()
    max_cov_scales =  np.round(np.linspace(1,3,5)[1:], 6).tolist()
    min_cov_scales =  np.round(np.linspace(1,3,5)[1:], 6).tolist()
    iters = 10
    
    base_params = list(product(std_dev_scales, bias_height_fractions, 
                            perturbations, optimizers, min_height_scales, max_height_scales, min_cov_scales, max_cov_scales, pert_type))
...
def single_run(args):
    run_id, std_scale, height_frac, perturb, opt, min_height_scale, max_height_scale, min_cov_scale, max_cov_scale, pert_type= args
    ...
    try:
        ...
        abc = CurvatureAdaptiveABC(
            ...
            perturb_type="fixed" if pert_type == 0 else "adaptive",
            ...
            min_bias_height=bias_height / min_height_scale,
            max_bias_height=bias_height * max_height_scale,

            bias_covariance_type="adaptive",
            default_bias_covariance=bias_cov,
            min_bias_covariance=bias_cov / min_cov_scale, 
            max_bias_covariance=bias_cov * max_cov_scale,
        ...
        run_data = {
            ...
            'min_height_scale':min_height_scale,
            'max_height_scale':max_height_scale,
            'min_cov_scale': min_cov_scale,
            'max_cov_scale': max_cov_scale,
            'pert_type': "fixed" if pert_type == 0 else "adaptive",
        }
