...
class StandardABC:
    ...
    def bfgs_refine_and_record(self):
        from scipy.optimize import minimize
        result = minimize(muller_brown, self.position, jac=compute_force, method='BFGS', options={'gtol': 1e-5})
        self.position = result.x
        self.minima.append(self.position)
    ...
    def run_simulation(self, max_steps=10000, verbose=True):
        ...
        for step in range(max_steps):
            ...
            if self.opt_type == "langevin": 
                self.langevin_step()
            else:
                self.bfgs_refine_and_record()
